...
var listenBlocks = function(config) {
    ...
    if(error) {
      ...
      if(chain3.isConnected()) {
          chain3.mc.getBlock(latestBlock, true, function(error,blockData) {
          if(error) {
...
var syncChain = function(config, nextBlock){
  if(chain3.isConnected()) {
    ...
    while(nextBlock >= config.startBlock && count > 0) {
        chain3.mc.getBlock(nextBlock, true, function(error,blockData) {
        if(error) {
    ...
    console.log('Error: chain3 connection time out trying to get block ' + nextBlock + ' retrying connection now');
...
var prepareSync = function(config, callback) {
  ...
  oldBlockFind.exec(function (err, docs) {
    if(err || !docs || docs.length < 1) {
      ...
      if(chain3.isConnected()) {
        var currentBlock = chain3.mc.blockNumber;
        ...
        console.log('Error: chain3 connection error');
        callback(err, null);
      }
    }else{
...
var runPatcher = function(config, startBlock, endBlock) {
  if(!chain3 || !chain3.isConnected()) {
    console.log('Error: chain3 is not connected. Retrying connection shortly...');
  ...
  if (missingBlocks > 0) {
    ...
    while(count < config.patchBlocks && patchBlock <= endBlock) {
      if(!('quiet' in config && config.quiet === true)) {
        ...
        chain3.mc.getBlock(patchBlock, true, function(error, patchData) {
        if(error) {
...
// Sets address for RPC chain3 to connect to, usually your node IP address defaults ot localhost
